...
$.globals = {
  username: localStorage.getItem("authUser") || $.globals?.username || null,
  loggedIn: true,
  lastTimeSentMsg: 0,
  isLoadingMsgs: 0,
  longPressTimer: null,
...
var loadMsgsFromServerByContactId = async function (
  ...
  //   console.log("DEBUG get_msgs request", { username: $username, contactId: $contactId, limit: $limit });
  ...
  postToServer({
    ...
    successCallback: function (data) {
      if (!data || data.length == 0) {
        if (!$clearChatIfEmpty) {
          console.warn("⚠️ No messages found for contact:", $contactId, "data returned:", data);
          return false;
        }
...
var sendTxtMsg = async function ($msg = null, $contactId = null, $username = null, $time = 0) {
  ...
  if (!$.settings.allowSendingMsgs) {
    consoleLog("you're trying to call sendTxtMsg while $.settings.allowSendingMsgs is false", {
      level: 5,
      type: "error",
    });
...
var getProfilePicByContactId = async function ($contactId = null, $username = null) {
  ...
  console.log("getProfilePicByContactId fired with $contactId", $contactId, { level: 3 });
  ...
  postToServer({
    ...
    successCallback: function (data) {
      ...
      data.forEach(function ($msg) {
        var $msgId = $msg["row_id"];
        // תוודא שההודעה לא קיימת כבר בדום
        if ($("#msg_id_" + $msgId).length === 0) {
          $html += proccessMsgsArr([$msg]);
        }
      });
        ...
        yes: {
          ...
          action: async function () {
            try {
              ...
              // Clean any client storage you’re using
              localStorage.removeItem("authToken");
              localStorage.removeItem("authTokenExp");
              localStorage.removeItem("authUser");
            } catch (e) {
              // Ignore network errors, we'll hard-redirect anyway
            } finally {
              // Hard redirect (cookie will be gone)
              window.location.href = "http://localhost:3000"; // or http://localhost:3000/
            }
          },
        },
        no: {
  ...
  $("#image_upload").on("change", function () {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        loadMsgsFromServerByContactId(); // reload chat
      },
    });
  });
});
